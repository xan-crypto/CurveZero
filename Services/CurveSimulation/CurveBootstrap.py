...
import os
os.chdir('C:/Users/Xan/PycharmProjects/Cario/Ftx')
# os.chdir('C:/Users/Xan/PycharmProjects/Cario/Deribit')
...
def get_aave_on(_yc):
    ...
    _yc = _yc.append({'source': 'aave','type': 'on','term': 0,'rate': int(response['data']['reserves'][0]['variableBorrowRate']) / 10 ** 27}, ignore_index=True)
    return _yc
...
def get_ftx_futs(_yc):
    ...
    for ticker in futures['result']:
        if ticker['underlying'] == 'BTC' and ticker['type'] == 'future':
            ...
            # spots futs basis is lend side, to get borrow side assume 20% of realised cash needed for margin
            if term > 0:
                rate = ((mid_price/index)**(365/term)-1)/0.85
...
df = yc[['term','rate']]
df.at[0,'rate'] = df.at[0,'rate']*margin
df.loc[:,'rate'] = df['rate'].values/margin
df = df.loc[df.rate >= df.iloc[0]['rate']].reset_index(drop=True)
df = df.loc[(df.term == 0)|(df.term >= 45)].reset_index(drop=True)
end_pt = int(df.iloc[-1]['term'])+1
...
df['days'] = range(0,end_pt)
df['rate'] = linFn(df['days'].values)

# save data in right format
df.to_json('test_rates.json', orient='table',index=False)
"""